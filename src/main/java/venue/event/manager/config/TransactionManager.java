...
public class TransactionManager {
    ...
    public <T> T inReadOnly(Function<Connection,T> work){
        ...
        try(Connection conn = dataSource.getConnection()){
            ...
            try{
                ...
                restoreReadOnlyConnectionState(conn,oldAutoCommit,oldReadOnly,failure);
            }
        }catch (SQLException e){
    ...
    private static void restoreReadOnlyConnectionState(Connection conn, boolean autoCommit,
                                                       boolean readOnly, Throwable primaryFailure) {
        TransactionException cleanupFailure = null;
        try {
            conn.setAutoCommit(autoCommit);
        } catch (SQLException failure) {
            if (primaryFailure != null) primaryFailure.addSuppressed(failure);
            else cleanupFailure = new TransactionException("Couldn't restore auto-commit", failure);
        }
        ...
        try {
            ...
            if (primaryFailure != null) primaryFailure.addSuppressed(failure);
            else if (cleanupFailure != null) cleanupFailure.addSuppressed(failure);
            else cleanupFailure = new TransactionException("Couldn't restore read-only state", failure);
        }
