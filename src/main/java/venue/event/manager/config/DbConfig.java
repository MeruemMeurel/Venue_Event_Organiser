...
public final class DbConfig {
    ...
    public DbConfig(String host, int port, String dbName, String user, String password, String sslMode,
        ...
        this.sslMode = requireValidSslMode(sslMode);
        this.schema = requireValidSchema(schema);
    }
    ...
    public String getJdbcUrl(){
        return String.format("jdbc:postgresql://%s:%d/%s?sslmode=%s&currentSchema=%s",
                host, port, dbName, sslMode, schema);
    }
    ...
    private static int parsePort(String portStr) {
        if(portStr == null || portStr.isBlank()) return 5432; // default PostgreSQL port
        try{
    ...
    private static String requireValidSslMode(String sslMode){
        String value = (sslMode == null || sslMode.isBlank()) ? "disable" : sslMode.trim();
        if (!value.matches("disable|allow|prefer|require|verify-ca|verify-full")) {
            throw new IllegalStateException("Invalid PostgreSQL SSL mode " + value);
        }
        return value;
    }

    private static String requireValidSchema(String schema){
        String value = (schema == null || schema.isBlank()) ? "public" : schema.trim();
        if (!value.matches("[A-Za-z_][A-Za-z0-9_$]*")) {
            throw new IllegalStateException("Invalid PostgreSQL schema " + value);
        }
        return value;
    }
