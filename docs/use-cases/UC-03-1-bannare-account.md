# UC-03.1 - Bannare un account

| Campo | Contenuto |
|---|---|
| **ID** | UC-03.1 |
| **Titolo** | Bannare un account |
| **Livello** | User goal |
| **Attore principale** | Administrator |
| **Attori secondari** | User |
| **Descrizione** | Un Administrator sospende un account ordinario attivo, impedendogli di utilizzare le operazioni non consentite agli utenti bannati. |
| **Trigger** | L'Administrator richiede di bannare uno User. |
...
1. Lo User si trova nello stato `BANNED`.
...
1. L'Administrator seleziona uno User attivo e richiede di bannarlo.
...
5. Il sistema verifica che lo User sia `ACTIVE`.
6. Il sistema aggiorna atomicamente lo stato a `BANNED`.
...
1. Il sistema rileva che lo User è già `BANNED`.
